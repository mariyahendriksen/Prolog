...
% ?- findall((Elem,Fib), (member(Elem,[5,10,15,20]),fibo(Elem,Fib)), Res). % uses fibo/2 from session2
% Res = [ (5, 5), (10, 55), (15, 610), (20, 6765)]

% GRAPHS
% ex1
% p(Graph, Node) - find all parents in Graph
% and marks the last one as 'end'.
p(Graph,Node) :-
...
% ex2
% all_parents(Graph, Node) - returns a list of all parents of the Node
all_parents(Gr,Parents):-
        findall(Parent,member(Parent/_Child,Gr),Parents).

% ex3
% shortest_path(X,Y,ShortestPath) - finds the shortest path between 2 nodes
% find all the paths and select the shortest one
% accumulator helps to compare paths lengthes and select the shortest one
memb(X,[X|_T]).
memb(X,[_H|T]):-
        memb(X,T).
...
shortest_path(X,Y,Gr,Res):-
        findall(Path,find_path(X,Y,Gr,Path),L),
        shortest(L,Res).
...
% MIXING PRODUCTS
/*
advice(Mixture) - returns a warning message about how this mixture is dangerous
5 results in no irritation,
between 6 and 12 results in minor irritation,
between 13 and 20 results in minor burning wounds,
between 21 and 30 results in severe burning wounds,
higher values can be lethal.
*/
% FINISH AT HOME
...
calc_reac([],Res,Res).
calc_reac([H|T],Acc,N):-
        findall(ReactVal, (memb(B,T), reaction(H,B,ReactVal) ), ReactL1 ),
        list_sum(ReactL1,Sum),
        Acc1 is Acc+Sum,
        calc_reac(T,Acc1,N).
...
% calculate eucleadean distance predicate
% DO AT HOME
...
manh_dist(L1,L2,Res):-
        manh_dist(L1,L2,0,Res).

manh_dist([],[],Res,Res).
manh_dist([H|T],[H1|T1],Acc,Res):-
        abs(H,H1,Diff),
        Acc1 is Acc+Diff,
        manh_dist(T,T1,Acc1,Res).
...
% Bottle - descr of bottle
classify(Bottle,Farmer):-
        findall(Dist/Farmer, (traindata(ChemDescr,Farmer), manh_dist(Bottle,ChemDescr,Dist)), AllDist),
        sort(AllDist,SortedDist),
        first_n_elem(5,SortedDist,NewSortedDist),
        pred_farmer(NewSortedDist,Farmer).
...
get_cand(L,Cand):-
        get_cand(L,[],Cand).
get_cand([],Res,Res).
get_cand([_A/B|T],Acc,Cand):-
        get_cand(T,[B|Acc],Cand).
